...
for genome in genomes :
    ...
    for i, L in enumerate(Lvals) :
        for j, p in enumerate(pvals) :
            ...
            for mapper in mappers :
                ...
                ax.loglog(roc[:,0], roc[:,1], 'o', label=mapper, alpha=0.5,
    ...
    fig.text(sup_xlabel_x, sup_xlabel_y, 'False positives', ha='center')
    fig.text(sup_ylabel_x, sup_ylabel_y, 'True positives', va='center', rotation='vertical')
    fig.savefig('%s/data/figures/%s-roc.pdf'%(sesame_bm_root,genome_aliases[genome]))
